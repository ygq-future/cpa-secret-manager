...
- **密钥管理**：列出、新增、删除代理密钥，密钥全程掩码显示并支持单条复制，按密钥或备注筛选；编辑只改备注（密钥由宿主持有，页面不可改也不回显明文）。
...
- 用量只统计命中受管密钥的请求；密钥为空或不属于任何受管密钥的记录一律丢弃，界面上不出现"未归因"之类的中间态。
...
打开 <http://localhost:8080/control-panel>。仿真宿主提供管理接口、同源 iframe 外壳与用量注入；外壳不渲染任何自身控件，**看到的就是插件页在真实宿主里的 1:1 效果**，主题与语言在控制台用 `setTheme(...)` / `setLanguage(...)` 切换，演示数据用 `seedDemo(3)`，详见 [`cmd/devserver/README.md`](cmd/devserver/README.md)。
